...
def parse_args_and_config():
    ...
    parser.add_argument('--strength', type=float, default=1e6, help='scale the gradients')
    parser.add_argument('--editList', type=list_of_ints, default=[10,40], help='timesteps between which edits should be done')
    parser.add_argument('--tagtime', type=int, default=0, help='timestep at which tags are fixed')
    parser.add_argument('--n_control', type=int, default=1, help='# of updates in each iteration')
    ...
    parser.add_argument('--text', nargs='+', help='List of strings for texts')
    ...
    if args.debias == "h_space":
        args.exp = args.exp + f'_debias_h_space_LC_{new_config.data.category}_t{args.t_0}_ninv{args.n_inv_step}_ngen{args.n_train_step}'
    else:
...
def main():
    ...
    print(args.text)
    args.proportions=text_to_ratio[len(args.text)]
    ...
    try:
        ...
        runner.run_test(texts=args.text)
