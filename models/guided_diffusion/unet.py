...
class UNetModel(nn.Module):
    ...
    def forward(self, x, alpha, text, strn, editList, tagtime, n_control, p, timesteps, clipModel, sample = True, male = 1, eyeglasses = 1, scale = [1500], timestep_list = [1,50], y=None, t_edit=400, hs_coeff=(1.0, 1.0), delta_h=None, ignore_timestep=False , use_mask=False, bt=1, attribute_list=[0,1,0,0], vanilla_generation=False, debias="vanilla"):
        """
        ...
        if debias == "h_space":
            n = h.shape[0]
            if conversion_timestep(editList[0]) <= timesteps.item() <= conversion_timestep(editList[1]):
            
                for _ in range(n_control):
                    if timesteps.item() >= conversion_timestep(tagtime):
                        h_c = h.clone()
                        hs_c = [hs_item.clone() for hs_item in hs]
                        for module in self.output_blocks:
                            h_c = th.cat([h_c, hs_c.pop()], dim=1)
                            h_c = module(h_c, emb)
                        h_c = h_c.type(xt.dtype)
                        eps = self.out(h_c)
                        eps = eps[:, :x.shape[1], ...]
                        x0_t = (xt - ((1 - alpha).sqrt() * eps)) / (alpha.sqrt())
                        batch = x0_t
                        
                        t0_list=text

                        tokens = clip.tokenize(t0_list).cuda()
                        image_input = clip_preprocess(x0_t)
                        
                        logits_per_image, _ = clipModel(image_input, tokens)
                        
                        
                        tags=np.zeros(n)
                        target = np.round(np.array(p) * n)
                        ...
                        self.tags = tags = tags.astype(int)
                    ...
                    with torch.enable_grad():
                        ...
                        for i in range(n):
                            logit, _ = clipModel(image_input[i].unsqueeze(0), tokens[self.tags[i]].unsqueeze(0))
                        ...
                        grads = torch.autograd.grad(outputs=loss, inputs=hc, grad_outputs=torch.ones_like(loss))[0]
